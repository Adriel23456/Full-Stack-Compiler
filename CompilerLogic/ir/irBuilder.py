...
UPDATE #10
• Se añade _visit_AssignmentExpressionContext : mismo comportamiento que
  _visit_AssignmentStatementContext pero sin requerir punto-y-coma.
  Esto actualiza efectivamente la variable de control dentro de los
  encabezados  loop(init; cond; incr)  evitando bucles infinitos.
• Resto del código idéntico al Update #9 (incluye soporte %).
"""
...
class IRGenerator:
    ...
    def __init__(self, ast, symtab, parser):
        ...
        # tipos LLVM
        self.f64 = ir.DoubleType(); self.i32 = ir.IntType(32)
        self.i1  = ir.IntType(1);   self.void = ir.VoidType()
    ...
    def generate(self) -> str:
        ...
        b = ir.IRBuilder(fn_wrap.append_basic_block("entry"))
        b.ret(b.call(fn_main, []))
    ...
    def _visit(self, node):
        ...
        for i in range(node.getChildCount()):
            v = self._visit(node.getChild(i)); res = res or v
        return res

    # ─────── TERMINALES ───────
    def _visit_TerminalNodeImpl(self, tok):
        t = tok.getText()
        if t in ("true","false"):     return ir.Constant(self.i1, t=="true")
        if t in COLORS:               return ir.Constant(self.i32, COLORS[t])
        if t.replace(".","",1).isdigit(): return ir.Constant(self.f64, float(t))
        return None
    ...
    def _binary(self, ctx):
        lhs,rhs = self._visit(ctx.getChild(0)), self._visit(ctx.getChild(2))
    ...
    def _visit_MulDivExprContext(self, ctx):
        ...
        if op == "%":
            lhs_i = self._round_to_i32(lhs); rhs_i = self._round_to_i32(rhs)
    ...
    def _visit_ComparisonExprContext(self, ctx):
        lhs,rhs = self._binary(ctx); op = ctx.getChild(1).getText()
        return self.builder.fcmp_ordered({"==":"oeq","!=":"one","<":"olt","<=":"ole",">":"ogt",">=":"oge"}[op], lhs,rhs)
    ...
    # ────────────────── STATEMENTS ────────────────────────────
    # asignación completa (con ';')
    def _visit_AssignmentStatementContext(self, ctx):
        self._handle_assignment(ctx.assignmentExpression())

    # EXPRESIÓN de asignación (sin ';')  – usada en encabezados loop()
    def _visit_AssignmentExpressionContext(self, ctx):
        return self._handle_assignment(ctx)

    # helper común
    def _handle_assignment(self, assign_ctx):
        name = assign_ctx.ID().getText()
        dest_ptr, dest_ty = self._get_ptr(name), self._get_ptr(name).type.pointee
    ...
    def _visit_SetColorStatementContext(self, ctx):
        raw = ctx.getChild(2)
        arg_val = self._visit(raw) or self.builder.load(self._get_ptr(raw.getText()))
    ...
    def _visit_WaitStatementContext (self, ctx):  self.builder.call(self.module.globals["vg_wait"], [self._round_to_i32(self._visit(ctx.expr()))])
    ...
    def _visit_IfStatementContext(self, ctx):
        ...
        self.builder.position_at_start(then_bb); self._visit(ctx.block(0)); self.builder.branch(merge_bb)
        if ctx.ELSE():
            self.builder.position_at_start(else_bb); self._visit(ctx.getChild(ctx.getChildCount()-1)); self.builder.branch(merge_bb)
        self.builder.position_at_start(merge_bb)

    def _visit_LoopStatementContext(self, ctx):
        self._visit(ctx.assignmentExpression(0))                # init
        f = self.builder.function
        cond_bb, body_bb, incr_bb, end_bb = [f.append_basic_block(n) for n in ("for.cond","for.body","for.incr","for.end")]
        self.builder.branch(cond_bb)

        self.builder.position_at_start(cond_bb)
        self.builder.cbranch(self._coerce(self._visit(ctx.boolExpr()), self.i1), body_bb, end_bb)

        self.builder.position_at_start(body_bb); self._visit(ctx.block()); self.builder.branch(incr_bb)

        self.builder.position_at_start(incr_bb); self._visit(ctx.assignmentExpression(1)); self.builder.branch(cond_bb)

        self.builder.position_at_start(end_bb)

    # ────────────────── helpers ───────────────────────────────
    def _coerce(self,val:ir.Value|None,target_ty:ir.Type)->ir.Value:
        if val is None: return self._const_zero(target_ty)
        if val.type==target_ty: return val
        if target_ty is self.i1:
            if val.type is self.f64: return self.builder.fcmp_ordered("one",val,ir.Constant(self.f64,0.0))
            if val.type is self.i32: return self.builder.icmp_unsigned("ne",val,ir.Constant(self.i32,0))
        if target_ty is self.i32:
            if val.type is self.f64: return self._round_to_i32(val)
            if val.type is self.i1:  return self.builder.zext(val,self.i32)
        if target_ty is self.f64:
            if val.type is self.i32: return self.builder.sitofp(val,self.f64)
            if val.type is self.i1:  return self.builder.uitofp(val,self.f64)
    ...
    def _round_to_i32(self,val:ir.Value)->ir.Value:
        if val.type is self.i32: return val
        if val.type is self.f64: return self.builder.fptosi(self.builder.fadd(val,ir.Constant(self.f64,0.5)), self.i32)
        if val.type is self.i1:  return self.builder.zext(val,self.i32)
        return ir.Constant(self.i32,0)

    def _const_zero(self,ty:ir.Type)->ir.Constant:
        if ty is self.f64: return ir.Constant(self.f64,0.0)
        if ty is self.i32: return ir.Constant(self.i32,0)
        if ty is self.i1:  return ir.Constant(self.i1 ,0)
    ...
    def _get_ptr(self,name:str)->ir.Value:
        ...
        if ptr is None:
            ...
            g = ir.GlobalVariable(self.module, self.f64, name); g.initializer = ir.Constant(self.f64,0.0)
            self.value_ptr[name]=g; ptr=g
        return ptr