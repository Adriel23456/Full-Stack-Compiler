#!/usr/bin/env python3
"""
HDMI_execute.py - Lanzador automático de vGraph y HDMI_visualizer
Este programa busca y ejecuta vGraph.exe y HDMI_visualizer.exe en su mismo directorio
"""
...
def check_files_exist(exe_dir):
    ...
    visualizer_path = os.path.join(exe_dir, "HDMI_visualizer.exe")
    ...
    # Verificar HDMI_visualizer.exe
    if not os.path.exists(visualizer_path):
        print(f"[Error] No se encontró HDMI_visualizer.exe")
        print(f"        Ruta esperada: {visualizer_path}")
        return False, None, None
    else:
        print(f"[OK] HDMI_visualizer.exe encontrado")
...
def main():
    ...
    # Lanzar HDMI_visualizer.exe
    print("[Client] Lanzando HDMI_visualizer.exe...")
    if launch_in_terminal(visualizer_path, "vGraph HDMI_visualizer"):
        print("[OK] HDMI_visualizer.exe lanzado correctamente")
    else:
        print("[Error] No se pudo lanzar HDMI_visualizer.exe")
        input("\nPresiona Enter para salir...")
        return 1
    ...
    print("✓ Los procesos vGraph y HDMI_visualizer continuarán ejecutándose")
    print("=" * 60)
