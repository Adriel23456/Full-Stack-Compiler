"""
Implementación de un analizador léxico, sintáctico y semántico utilizando PLY (Python Lex-Yacc)
con soporte para estructuras 'if-elif-else' y bucles 'for'.
Las fases de análisis están completamente separadas siguiendo el modelo de compilador tradicional.
"""
...
class Interpreter:
    def __init__(self, symbol_table):
        self.symbol_table = symbol_table.copy()  # Usar una copia de la tabla de símbolos del análisis semántico
    ...
    def _evaluate_node(self, node):
        ...
        # Si es una tupla, es un nodo interno del AST
        if isinstance(node, tuple):
            if node[0] == 'program':
                # Evaluar lista de declaraciones
                for statement in node[1]:
                    self._evaluate_node(statement)
            
            elif node[0] == 'assign':
                # Evaluar asignación
                var_name = node[1]
                value = self._evaluate_node(node[2])
                self.symbol_table[var_name] = value
                return value
            
            elif node[0] == 'binop':
                # Evaluar operación binaria
                left = self._evaluate_node(node[2])
                right = self._evaluate_node(node[3])
                ...
                if node[1] == '+':
                    ...
                    if right == 0:
                        print(f"Error en tiempo de ejecución: División por cero en línea {node[4]}")
                        return 0
                    return left / right
            
            elif node[0] == 'relop':
                # Evaluar operación relacional
                left = self._evaluate_node(node[2])
                right = self._evaluate_node(node[3])
                ...
                # Retornar valor de variable
                var_name = node[1]
                return self.symbol_table.get(var_name, 0)
            
            elif node[0] == 'print':
                # Evaluar y mostrar expresión
                value = self._evaluate_node(node[1])
                print(f"SALIDA: {value}")
                return value
            
            elif node[0] == 'if':
                # Evaluar condición if
                condition = self._evaluate_node(node[1])
                if condition:
                    # Ejecutar bloque if
                    for statement in node[2]:
                        self._evaluate_node(statement)
                elif node[3]:  # Hay un elif
                    # Evaluar condición elif
                    elif_condition = self._evaluate_node(node[3][1])
                    if elif_condition:
                        for statement in node[3][2]:
                            self._evaluate_node(statement)
                    elif node[4]:  # Hay un else después del elif
                        for statement in node[4]:
                            self._evaluate_node(statement)
                elif node[4]:  # Hay un else directo
                    for statement in node[4]:
                        self._evaluate_node(statement)
                ...
                # Evaluar bucle for
                var_name = node[1]
                range_end = self._evaluate_node(node[2])
                
                if not isinstance(range_end, int) or range_end < 0:
                    print(f"Error en tiempo de ejecución: El valor del rango debe ser un entero no negativo")
                    return None
        ...
        # Si es una lista, evaluar cada elemento
            ...
            results = []
            for item in node:
                results.append(self._evaluate_node(item))
            return results
...
def compile_and_run(source_code):
    """Función principal que compila y ejecuta el código fuente"""
    ...
    print("\n==== ANALIZADOR SINTÁCTICO ====")
    lexer.input(source_code)  # Reiniciar el lexer
    ...
    print("AST generado (estructura simplificada):")
    print_ast(ast, 0)
    ...
    print("Tabla de símbolos después del análisis semántico:")
    for var, info in semantic_analyzer.symbol_table.items():
        print(f"  {var}: declarada en línea {info['line']}")
    
    # Fase 4: Interpretación (ejecución)
    print("\n==== EJECUCIÓN ====")
    interpreter = Interpreter(semantic_analyzer.symbol_table)
    interpreter.evaluate(ast)
    
    print("\nTabla de símbolos final (después de la ejecución):")
    for var, value in interpreter.symbol_table.items():
        if not isinstance(value, dict):  # No imprimir las entradas de metadatos
            print(f"  {var} = {value}")

def print_ast(node, indent=0):
    """Imprime el AST de forma recursiva y legible"""
    indent_str = "  " * indent
    ...
    if isinstance(node, tuple):
        ...
        if node_type == 'program':
            ...
            print(f"{indent_str}{node_type}: {node[1]} = <expr>")
            print_ast(node[2], indent + 1)
        elif node_type == 'binop':
            print(f"{indent_str}{node_type}: {node[1]}")
            print_ast(node[2], indent + 1)
            ...
            print(f"{indent_str}{node_type}:")
            print(f"{indent_str}  condición:")
            print_ast(node[1], indent + 2)
            print(f"{indent_str}  cuerpo:")
            print_ast(node[2], indent + 2)
            if node[3]:  # elif
                print(f"{indent_str}  elif:")
                print(f"{indent_str}    condición:")
                print_ast(node[3][1], indent + 3)
                print(f"{indent_str}    cuerpo:")
                print_ast(node[3][2], indent + 3)
            if node[4]:  # else
                print(f"{indent_str}  else:")
                print_ast(node[4], indent + 2)
        elif node_type == 'for':
            print(f"{indent_str}{node_type}: variable={node[1]}")
            print(f"{indent_str}  rango:")
            print_ast(node[2], indent + 2)
            print(f"{indent_str}  cuerpo:")
            print_ast(node[3], indent + 2)
